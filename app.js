const bootEl=document.getElementById("boot"),titleStage=document.getElementById("titleStage"),masterTitle=document.getElementById("masterTitle"),shell=document.getElementById("shell"),greeting=document.getElementById("greeting"),tapGate=document.getElementById("tapGate");
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const bootLines=["FROZE-NEX SYSTEM INITIALIZATION","Loading FNX display subsystem...","Loading audio feedback modules...","Mounting user environment...","Checking interface state...","Starting FNX graphical shell...","Authentication granted."];
...
tapGate.classList.add("hidden");bootEl.textContent="";titleStage.classList.add("hidden");shell.classList.add("hidden");shell.classList.remove("open");greeting.classList.remove("visible");document.body.classList.remove("fade-out");
for(let i=0;i<bootLines.length;i++){const line=bootLines[i];play(line==="Authentication granted."?"granted":"stdout");bootEl.textContent+=line+"\n";if(i===1)bootEl.textContent+="FNX Kernel boot at "+new Date().toString()+"; root:xnu-1699.22.73~1/RELEASE_ARM64\n";await sleep(bootDelay(i+1,bootLines.length))}
...
window.location.href="shortcuts://run-shortcut?name=FROZE-NEX%20END";
}
let started=false;
